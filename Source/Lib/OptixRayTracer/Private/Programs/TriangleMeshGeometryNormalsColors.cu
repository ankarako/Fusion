...
rtDeclareVariable(optix::float3, triangle_color, attribute TriangleColor, );
...
RT_PROGRAM void triangle_mesh_intersect(int primIdx)
{
	...
	if (optix::intersect_triangle(ray, v0, v1, v2, normal, t, beta, gamma))
	{
		if (rtPotentialIntersection(t))
		{
			...
			optix::float3 c0 = colors_buffer[vertexIdx.x];
			optix::float3 c1 = colors_buffer[vertexIdx.y];
			optix::float3 c2 = colors_buffer[vertexIdx.z];
			triangle_color = c1 * beta + c2 * gamma + c0 * (1.0f - beta - gamma);
