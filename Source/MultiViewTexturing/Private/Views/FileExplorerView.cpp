...
struct FileExplorerView::Impl
{
	...
	rxcpp::subjects::subject<std::string>		ImportPerformanceFlowOutSubj;
...
void FileExplorerView::Render() 
{
	...
	ImGui::Begin("File explorer");
	{
		...
		else if (m_Impl->m_Mode == FileExplorerMode::ImportPerformance)
		{
			if (ImGui::Button("Import"))
			{
				...
				std::string abspath = m_Impl->m_CurrentDirectory + "/" + name;
				DirEntry perfEntry;
				perfEntry.EntryType = DirEntry::Type::File;
				perfEntry.AbsPath = abspath;
				perfEntry.Name = name;
				m_Impl->ImportPerformanceFlowOutSubj.get_subscriber().on_next(perfEntry.AbsPath);
				memset(m_Impl->m_InputTextBuffer, 0, m_Impl->k_InputBufferSize * sizeof(char));
				this->Deactivate();
			}
		}
...
rxcpp::observer<std::string> mvt::FileExplorerView::CurrentDirectoryFlowIn()
{
	return m_Impl->CurrentDirectoryFlowInSubj.get_subscriber().get_observer().as_dynamic();
}

rxcpp::observer<FileExplorerView::dir_entry_array> mvt::FileExplorerView::CurrentDirEntriesFlowIn()
{
	return m_Impl->CurrentDirEntriesFlowInSubj.get_subscriber().get_observer().as_dynamic();
}

rxcpp::observer<FileExplorerView::dir_entry_array> mvt::FileExplorerView::CurrentDirHierarchyIn()
{
	return m_Impl->CurrentDirHierarchyInSubj.get_subscriber().get_observer().as_dynamic();
}

rxcpp::observer<unsigned int> mvt::FileExplorerView::DriveCountFlowIn()
{
	return m_Impl->DriveCountFlowInSubj.get_subscriber().get_observer().as_dynamic();
}

rxcpp::observer<FileExplorerView::drive_letter_array> mvt::FileExplorerView::DriveLettersFlowIn()
{
	return m_Impl->DriveLettersFlowInSubj.get_subscriber().get_observer().as_dynamic();
}

rxcpp::observable<DirEntry> mvt::FileExplorerView::OpenProjectFileFlowOut()
{
	return m_Impl->OpenProjectFileSubj.get_observable().as_dynamic();
}

rxcpp::observable<DirEntry> mvt::FileExplorerView::SaveProjectFileFlowOut()
{
	return m_Impl->SaveProjectFileSubj.get_observable().as_dynamic();
}

rxcpp::observable<std::string> mvt::FileExplorerView::OpenVideoFileFlowOut()
{
	return m_Impl->OpenVideoFileSubj.get_observable().as_dynamic();
}

rxcpp::observable<void*> mvt::FileExplorerView::OnMoveUpButtonClicked()
{
	return m_Impl->OnMoveUpButtonClickedSubj.get_observable().as_dynamic();
}

rxcpp::observable<std::string> mvt::FileExplorerView::OnMoveIntoDirectory()
{
	return m_Impl->OnMoveIntoClickedSubj.get_observable().as_dynamic();
}

rxcpp::observer<std::string> mvt::FileExplorerView::CurrentSelectedDriveFlowIn()
{
	return m_Impl->CurrentSelectedDriveFlowInSubj.get_subscriber().get_observer().as_dynamic();
}
rxcpp::observable<std::string> fu::mvt::FileExplorerView::Open3DFileFlowOut()
{
	return m_Impl->Open3DFileSubj.get_observable().as_dynamic();
}
rxcpp::observable<std::string> fu::mvt::FileExplorerView::OpenPerfcapFileFlowOut()
{
	return m_Impl->OpenPerfcapFileSubj.get_observable().as_dynamic();
}

rxcpp::observable<std::string> mvt::FileExplorerView::ImportPerformanceFlowOut()
{
	return m_Impl->ImportPerformanceFlowOutSubj.get_observable().as_dynamic();
}
}	///	!namespace mvt
}	///	!namespace fu